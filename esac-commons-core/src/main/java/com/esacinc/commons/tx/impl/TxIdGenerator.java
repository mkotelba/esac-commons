...
public class TxIdGenerator implements IdGenerator, InitializingBean, NamedBean {
    private final static long NUM_INTERVALS_PER_MS = (EsacDateUtils.NS_IN_MS / 100L);
    ...
    private final static String DIGEST_ALG_NAME = "SHA-1";

    private final static Provider DIGEST_PROV = Security.getProvider("SUN");
    ...
    public TxIdGenerator(String name) {
        this.name = name;
    }
    ...
    public void afterPropertiesSet() throws Exception {
        (this.nodeId =
            ArrayUtils.subarray((this.nodeId = MessageDigest.getInstance(DIGEST_ALG_NAME, DIGEST_PROV).digest(name.getBytes(StandardCharsets.UTF_8))),
