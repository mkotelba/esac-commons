...
import com.esacinc.commons.transform.saxon.EsacReceiverOptions;
...
public abstract class AbstractEsacCdataXmlFilter extends ProxyReceiver {
    protected XMLStreamWriter2 writer;
    protected boolean inCdataElem;

    protected AbstractEsacCdataXmlFilter(Receiver nextReceiver) {
        super(nextReceiver);
    ...
    public void characters(CharSequence chars, Location loc, int props) throws XPathException {
        ...
        if ((props & ReceiverOptions.DISABLE_ESCAPING) != 0) {
            ...
            try {
                if ((props & EsacReceiverOptions.USE_CDATA) != 0) {
            ...
            super.characters(chars, loc, (props & ~EsacReceiverOptions.USE_CDATA));
        }
    }
