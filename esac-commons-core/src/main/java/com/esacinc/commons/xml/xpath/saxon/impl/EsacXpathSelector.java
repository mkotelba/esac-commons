...
import com.esacinc.commons.xml.saxon.utils.EsacXdmUtils;
...
public class EsacXpathSelector extends XPathSelector {
    public static class EsacXdmSequenceIterator extends XdmSequenceIterator {
        private SequenceIterator seqIterator;

        public EsacXdmSequenceIterator(SequenceIterator seqIterator) {
            super(seqIterator);
    ...
    public EsacXpathSelector(XPathExpression expr, StaticXpathOptions staticOpts, DynamicXpathOptions dynamicOpts, XPathDynamicContext dynamicContext,
    ...
    public List<XdmNode> evaluateNodes() throws SaxonApiException {
        return EsacXdmUtils.streamNodes(this.iterator()).collect(Collectors.toList());
    }

    public List<String> evaluateStrings() throws SaxonApiException {
        return EsacXdmUtils.streamStrings(this.iterator()).collect(Collectors.toList());
    }

    public List<XdmAtomicValue> evaluateAtomicValues() throws SaxonApiException {
        return EsacXdmUtils.streamAtomicValues(this.iterator()).collect(Collectors.toList());
    }
    ...
    @Override
    public EsacXdmSequenceIterator iterator() {
        try {
            return new EsacXdmSequenceIterator(this.expr.iterate(this.dynamicContext));
        } catch (XPathException e) {
